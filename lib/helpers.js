...
module.exports = function(config){
  ...
  return {
     ...
     * @return
     */
    timeCorrection: function(response, json){
      if( !json ) return null;

      var serverDateString = response.headers.date;
      if(!serverDateString) return;

      var serverDate = new Date(serverDateString);
      var now = new Date();
      var diff = now - serverDate;

      this._timeCorrection(diff, json);
    },

    _timeCorrection: function(timeDiff, json){
      if( !json ) return null;

      var dateFieldList = ["modified", "client_mtime"];

      dateFieldList.forEach(function(field){
        if(typeof json[field] == 'undefined')return;

        var value = json[field];
        var time = new Date(value).getTime();

        var coTime = time + timeDiff;
        json[field] = new Date(coTime).toString();
      ...
      Object.keys(json).forEach(function(key){
        ...
        if(typeof subJSON === 'object'){
          self._timeCorrection(timeDiff, subJSON);
        }
      });
    },
  }
